...
#              : mar 08 2016 - Rewrite of entire script               (T.Boot)
#              : mar 09 2016 - Added function for description in conf (T.Boot)
#              : mar 10 2016 - Added logging                          (T.Boot)
#              : mar 14 2016 - Fixed profile check in silent profile  (T.Boot)
# -------------------------------------------------------------------------- #
# Extra        : 
# -------------------------------------------------------------------------- #
...
#      --- Variables ---
#----------------------------------------------------------------------------#
scriptVersion="2.0.2"										# script version
	...
	case "$1" in
		...
		server)
			for srvName in ${!serverNames[*]}; do
				if [[ "${serverNames[$srvName]}" =~ "?" ]]; then
					sName=`echo ${serverNames[$srvName]} | cut -d? -f2`
				else
...
if [[ "$paramName" == "-s" ]]; then
	...
	serverListCheck=`grep -H ${paramName} ${cfgDir}/asmlsrv.txt`
...
if [ "${serverList}" == "list" ]; then
	...
	if [ -z "${serverNames[*]}" ]; then
		...
		echo "servName= ${serverNames[$addressID]}"
